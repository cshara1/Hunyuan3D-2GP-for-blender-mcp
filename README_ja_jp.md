...
## 🔌 Blender MCP 統合
`api_server.py` は [blender-mcp](https://github.com/deepbeepmeep/blender-mcp) とのシームレスな統合のために更新されました。これにより、Model Context Protocol を使用して Blender から直接 3D 生成を実行できるようになります。

### API サーバーのパラメータ
- `--host`: サーバーを実行するホスト (デフォルト: 127.0.0.1)
- `--port`: サーバーを実行するポート (デフォルト: 8081)
- `--share`: サーバーを公開する (デフォルト: False)
