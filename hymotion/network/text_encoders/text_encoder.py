    ...
    # Note: Qwen3 model loading is handled by trust_remote_code=True in the from_pretrained calls below.
    # The model's custom code will be downloaded from Hugging Face Hub automatically.
    ...
    # --- Monkeypatch to allow 'qwen3' model type ---
    try:
        from transformers import AutoConfig, Qwen2Config
        from transformers.models.auto.configuration_auto import CONFIG_MAPPING
        
        # Register 'qwen3' to use Qwen2Config
        # We must update the CONFIG_MAPPING dict directly to bypass the KeyError
        if "qwen3" not in CONFIG_MAPPING:
            CONFIG_MAPPING["qwen3"] = Qwen2Config
        
        # Also use official register API just in case
        AutoConfig.register("qwen3", Qwen2Config)
        print(">>> Successfully registered 'qwen3' model type alias to Qwen2Config")
    except Exception as e:
        print(f"Warning: Failed to register qwen3 config: {e}")
    # -----------------------------------------------
        ...
        def __init__(
            ...
            if llm_type is not None:
                ...
                if quantization == "int4":
                    ...
                    # Manually load config to handle qwen3 model type
                    try:
                        config = AutoConfig.from_pretrained(LLM_ENCODER_LAYOUT[llm_type]["module_path"], trust_remote_code=True)
                    except:
                        config = None
